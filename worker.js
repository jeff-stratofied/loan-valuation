...
// Helper: Save JSON to GitHub (fixed version)
async function saveToGitHub(env, path, content, oldSha, commitMsg) {
  ...
  } // else: if file didn't exist, proceed without sha
...
async function handleFetch(request, env) {
  ...
  try {
    ...
    // BORROWERS (example — add similar if needed)
    if (url.pathname === "/borrowers") {
      // ... your existing code ...
    }

    // SCHOOL TIERS, VALUATION CURVES (read-only)
    if (url.pathname === "/schoolTiers" || url.pathname === "/valuationCurves") {
      if (request.method === "GET") {
